...
/**
 * Demo 19: Strong Center Bias + Brightness Gradient
 * pow(0.25) center bias + brightness scales strongly with proximity to center.
 * Additive blending amplifies overlap at center -> organic halo.
...
function ParticleConstellation({ mouseRef }: { mouseRef: MouseRef }) {
  ...
  const { positions, colors, basePositions } = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Strong brightness gradient: center particles much brighter
...
export default function HeroParticlesHaloADemo() {
  ...
  return (
    <DemoSection
      number={19}
      title="Halo A — Forte concentration centre + brightness"
      description="pow(0.25) center bias + brightness +80% au centre. Pas de masque radial — distribution complete visible."
    >
      <div
